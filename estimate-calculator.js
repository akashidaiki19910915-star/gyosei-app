(function () {
  ...
  function init(){
    ...
    root.querySelector('#calc-apply').addEventListener('click', async()=>{const app=window.GyoseiApp; const sb=app?.getSupabaseClient?.(); const u=app?.getCurrentUser?.(); if(!sb||!u)return; run(); const r=JSON.parse(form.dataset.result||'{}'); const f=form.elements;
      const est={user_id:u.id,client_id:f.clientId.value||null,customer_name:'自動算出',estimate_title:f.projectName.value||'見積自動算出',estimate_date:new Date().toISOString().slice(0,10),status:'未回答',memo:f.memo.value||null,subtotal:r.taxable||0,tax:r.tax||0,total:r.taxable+r.tax};
