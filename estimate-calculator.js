  ...
  async function init() {
    ...
    const loadSavedCalculations = async () => {
      ...
      try {
        ...
        if (!sessionData?.session?.access_token || sessionData.session.user?.id !== u.id) {
          localEstimateCalculations = [];
          return;
        }
        ...
        debugLog("LOAD ESTIMATE CALCULATIONS RESULT", { count: localEstimateCalculations.length });
      } catch (error) {
